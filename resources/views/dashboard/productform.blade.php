<x-dashboard>
    <x-slot name="title">{{ !isset($product) ? "Create Blog" : "Edit Blog " . $product->title }}</x-slot>
    ...
    <x-slot name="content">
        <form class="bg-light" style="border: 0.1px solid gray; padding: 2rem; box-shadow: 2px 2px 2px 2px gray;"
         action="{{ route("CreateBlog") }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <textarea class="form-control" name="description" id="" cols="30" rows="10" placeholder="Product Description">
